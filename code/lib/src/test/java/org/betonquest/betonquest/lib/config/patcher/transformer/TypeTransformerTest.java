...
class TypeTransformerTest extends TransformersFixture {
    ...
    void throws_exception_on_invalid() {
        assertThrows(PatchException.class, () -> TRANSFORMER.transform(new DefaultPatcherOptions(Map.of("key", "section.invalid", "newType", "boolean")), config));
    }

    @Test
    void throws_exception_on_unsupported_type() {
        assertThrows(PatchException.class, () -> TRANSFORMER.transform(new DefaultPatcherOptions(Map.of("key", "section.boolean", "newType", "invalid")), config));
    }
}
