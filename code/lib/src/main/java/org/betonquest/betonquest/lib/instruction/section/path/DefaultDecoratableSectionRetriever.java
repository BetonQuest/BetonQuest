...
public class DefaultDecoratableSectionRetriever<T> extends DefaultSectionRetriever<T> implements DecoratableSectionRetriever<T> {
     ...
     */
    public DefaultDecoratableSectionRetriever(final SectionChainInstruction instruction, final String rootPath, final InstructionArgumentParser<T> parser) {
    ...
    public ListSectionRetriever<T> list() {
        return new DefaultListSectionRetriever<>(instruction, rootPath, decoratable().list());
    }

    @Override
    public <R> DecoratableSectionRetriever<R> collect(final Collector<T, ?, R> collector) {
        return new DefaultDecoratableSectionRetriever<>(instruction, rootPath, decoratable().collect(collector));
    }

    @Override
    public <U> DecoratableSectionRetriever<U> map(final QuestFunction<T, U> mapper) {
        return new DefaultDecoratableSectionRetriever<>(instruction, rootPath, decoratable().map(mapper));
    }

    @Override
    public DecoratableSectionRetriever<T> validate(final ValueValidator<T> validator) {
        return new DefaultDecoratableSectionRetriever<>(instruction, rootPath, decoratable().validate(validator));
    }

    @Override
    public DecoratableSectionRetriever<T> validate(final ValueValidator<T> validator, final String errorMessage) {
        return new DefaultDecoratableSectionRetriever<>(instruction, rootPath, decoratable().validate(validator, errorMessage));
    }

    @Override
    public DecoratableSectionRetriever<T> prefilter(final String expected, final T fixedValue) {
        return new DefaultDecoratableSectionRetriever<>(instruction, rootPath, decoratable().prefilter(expected, fixedValue));
    }

    @Override
    public DecoratableSectionRetriever<Optional<T>> prefilterOptional(final String expected, final T fixedValue) {
        return new DefaultDecoratableSectionRetriever<>(instruction, rootPath, decoratable().prefilterOptional(expected, fixedValue));
    }
}
