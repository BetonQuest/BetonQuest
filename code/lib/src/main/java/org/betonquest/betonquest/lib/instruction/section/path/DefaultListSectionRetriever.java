...
public class DefaultListSectionRetriever<T> extends DefaultDecoratableSectionRetriever<List<T>> implements ListSectionRetriever<T> {
     ...
     */
    public DefaultListSectionRetriever(final SectionChainInstruction instruction, final String rootPath, final InstructionArgumentParser<List<T>> parser) {
    ...
    public ListSectionRetriever<T> prefilter(final String expected, final List<T> fixedValue) {
        return new DefaultListSectionRetriever<>(instruction, rootPath, decoratable().prefilter(expected, fixedValue));
    }

    @Override
    public ListSectionRetriever<T> validate(final ValueValidator<List<T>> validator) {
        return new DefaultListSectionRetriever<>(instruction, rootPath, decoratable().validate(validator));
    }

    @Override
    public ListSectionRetriever<T> validate(final ValueValidator<List<T>> validator, final String errorMessage) {
        return new DefaultListSectionRetriever<>(instruction, rootPath, decoratable().validate(validator, errorMessage));
    }

    @Override
    public ListSectionRetriever<T> notEmpty() {
        return new DefaultListSectionRetriever<>(instruction, rootPath, decoratable().notEmpty());
    }

    @Override
    public ListSectionRetriever<T> distinct() {
        return new DefaultListSectionRetriever<>(instruction, rootPath, decoratable().distinct());
    }

    @Override
    public <U> ListSectionRetriever<T> distinct(final Function<T, U> extractor) {
        return new DefaultListSectionRetriever<>(instruction, rootPath, decoratable().distinct(extractor));
    }
}
