...
/**
 * A collection of arguments representing a mix step argument.
 *
 ...
 */
public record MixArgument(Argument<CauldronType> cauldronTypeArgument, Argument<Number> cookTimeArgument,
     ...
     * @return A mix argument
     * @throws QuestException If any of the arguments are invalid
     */
    public static MixArgument fromChain(final Instruction instruction, final IngredientManager<?> ingredientManager) throws QuestException {
        ...
        return new MixArgument(cauldronTypeArgument, cookTimeArgument, ingredientsArgument, ingredientManager);
    }
    ...
    public QuestFunction<Brew, Boolean> getValue(@Nullable final Profile profile) throws QuestException {
        ...
        return brew -> {
            ...
            try {
                if (!(brew.lastCompletedStep() instanceof final BrewingStep.Mix mixStep)) {
                    return false;
                }
                return mixStep.time().moment() > cookTime * Moment.MINUTE
                        && cauldronType.appliesTo(mixStep.cauldronType())
                        && IngredientsUtil.checkMatch(ingredientsFuture.join(), mixStep.ingredients());
