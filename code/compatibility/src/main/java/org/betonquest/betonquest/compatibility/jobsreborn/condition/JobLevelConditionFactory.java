...
/**
 * Factory to create {@link JobLevelCondition}s from {@link DefaultInstruction}s.
 */
public class JobLevelConditionFactory implements PlayerConditionFactory {
    ...
    public PlayerCondition parsePlayer(final DefaultInstruction instruction) throws QuestException {
        ...
        final Variable<Number> minimum = instruction.get(Argument.NUMBER);
        final Variable<Number> maximum = instruction.get(Argument.NUMBER);
