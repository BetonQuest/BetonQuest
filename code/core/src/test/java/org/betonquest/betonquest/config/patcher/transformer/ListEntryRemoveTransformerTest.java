...
class ListEntryRemoveTransformerTest extends TransformersFixture {
    ...
    void flawless() throws PatchException {
        ...
        TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.myList", "entry", "removedEntry")), config);
    ...
    void throws_exception_on_invalid() {
        assertThrows(PatchException.class, () -> TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.invalid", "entry", "removedEntry")), config));
    }

    @Test
    void throws_exception_on_no_matching_entry() {
        assertThrows(PatchException.class, () -> TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.myList", "entry", "invalidEntry")), config));
    }
}
