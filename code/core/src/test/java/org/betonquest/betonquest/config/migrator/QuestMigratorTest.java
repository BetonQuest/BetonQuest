...
class QuestMigratorTest extends QuestFixture {
    ...
    void version_no_set() throws IOException, InvalidConfigurationException, VersionMissmatchException {
        ...
        final Version currentVersion = VersionParser.parse(BetonQuestMigratorVersion.QUEST_PACKAGE_VERSION_TYPE, "3.2.1-QUEST-5");
        new QuestMigrator(logger, Collections.emptyList(), Collections.emptyMap(), currentVersion).migrate(quest);
        assertEquals(currentVersion.toString(), quest.getQuestConfig().getString(VERSION_PATH),
                "The version should be set in the quest if none is present.");
        assertEquals(currentVersion.toString(), loadPackageFile().getString(VERSION_PATH),
    ...
    void version_older_set() throws IOException, InvalidConfigurationException, VersionMissmatchException {
        ...
        final Quest quest = setupQuest();
        final Version fallback = VersionParser.parse(BetonQuestMigratorVersion.QUEST_PACKAGE_VERSION_TYPE, "3.0.0-QUEST-0");
        new QuestMigrator(logger, Collections.emptyList(), Collections.emptyMap(), fallback).migrate(quest);
        assertEquals(fallback.toString(), quest.getQuestConfig().getString(VERSION_PATH),
                "Older versions should be updated in the quest.");
        assertEquals(fallback.toString(), loadPackageFile().getString(VERSION_PATH),
                "Older versions should be updated in the file.");
    }

    @Test
    void version_newer_set() throws IOException, InvalidConfigurationException {
        final Version qustVersion = VersionParser.parse(BetonQuestMigratorVersion.QUEST_PACKAGE_VERSION_TYPE, "3.0.1-QUEST-0");
        original.set(VERSION_PATH, qustVersion.toString());
        final Quest quest = setupQuest();
        final Version fallback = VersionParser.parse(BetonQuestMigratorVersion.QUEST_PACKAGE_VERSION_TYPE, "3.0.0-QUEST-0");
    ...
    void no_legacy_migration() throws IOException, InvalidConfigurationException, VersionMissmatchException {
        ...
        final Quest quest = setupQuest();
        final Version fallback = VersionParser.parse(BetonQuestMigratorVersion.QUEST_PACKAGE_VERSION_TYPE, "3.0.0-QUEST-1");
    ...
    void legacy_migration() throws IOException, InvalidConfigurationException, VersionMissmatchException {
        ...
        final Map<Version, QuestMigration> migrationMap = Map.of(VersionParser.parse(BetonQuestMigratorVersion.QUEST_PACKAGE_VERSION_TYPE, "2.3.4-QUEST-7"), versioned);
    ...
    void select_migration() throws IOException, InvalidConfigurationException, VersionMissmatchException {
        ...
        final Version newestMigrationVersion = VersionParser.parse(BetonQuestMigratorVersion.QUEST_PACKAGE_VERSION_TYPE, "3.0.1-QUEST-2");
        final Map<Version, QuestMigration> migrationMap = Map.of(
                VersionParser.parse(BetonQuestMigratorVersion.QUEST_PACKAGE_VERSION_TYPE, "2.3.4-QUEST-1"), older,
                newestMigrationVersion, newer
        );
        final Version fallback = VersionParser.parse(BetonQuestMigratorVersion.QUEST_PACKAGE_VERSION_TYPE, "3.2.0-QUEST-1");
    ...
    void set_migration_version() throws IOException, InvalidConfigurationException, VersionMissmatchException {
        original.set(VERSION_PATH, "3.0.0-QUEST-0");
        final Version migrationVersion = VersionParser.parse(BetonQuestMigratorVersion.QUEST_PACKAGE_VERSION_TYPE, "3.0.1-QUEST-2");
        final Map<Version, QuestMigration> migrationMap = Map.of(migrationVersion, mock(QuestMigration.class));
        final Version fallback = VersionParser.parse(BetonQuestMigratorVersion.QUEST_PACKAGE_VERSION_TYPE, "3.2.0-QUEST-1");
        final Quest quest = setupQuest();
        new QuestMigrator(logger, Collections.emptyList(), migrationMap, fallback).migrate(quest);
        assertEquals(migrationVersion.toString(), quest.getQuestConfig().getString(VERSION_PATH),
