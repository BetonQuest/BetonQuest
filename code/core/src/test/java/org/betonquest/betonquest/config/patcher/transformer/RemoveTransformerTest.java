...
class RemoveTransformerTest extends TransformersFixture {
    ...
    void flawless() throws PatchException {
        TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.myList")), config);
        assertNull(config.get("section.myList"), "The list was not removed.");
    }

    @Test
    void throws_exception_on_invalid() {
        assertThrows(PatchException.class, () -> TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.invalid")), config));
    }
}
