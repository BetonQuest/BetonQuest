...
import org.betonquest.betonquest.config.patcher.PatcherOptionsImpl;
...
class TypeTransformerTest extends TransformersFixture {
    ...
    void flawless_String_to_Boolean() throws PatchException {
        TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.booleanString", "newType", "boolean")), config);
    ...
    void flawless_String_to_Int() throws PatchException {
        TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.int", "newType", "integer")), config);
    ...
    void flawless_String_to_Float() throws PatchException {
        TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.float", "newType", "float")), config);
    ...
    void flawless_String_to_Double() throws PatchException {
        TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.double", "newType", "double")), config);
    ...
    void flawless_Boolean_to_String() throws PatchException {
        TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.boolean", "newType", "string")), config);
        assertEquals(config.get("section.boolean"), "true", "The boolean was not converted to a string.");
    }

    @Test
    void throws_exception_on_invalid() {
        assertThrows(PatchException.class, () -> TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.invalid", "newType", "boolean")), config));
    }

    @Test
    void throws_exception_on_unsupported_type() {
        assertThrows(PatchException.class, () -> TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.boolean", "newType", "invalid")), config));
    }
}
