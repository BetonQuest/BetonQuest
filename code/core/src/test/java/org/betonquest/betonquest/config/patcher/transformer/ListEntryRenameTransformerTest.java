...
class ListEntryRenameTransformerTest extends TransformersFixture {
    ...
    void flawless() throws PatchException {
        ...
        TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.myList", "oldEntryRegex", "currentEntry", "newEntry", "newEntry")), config);
    ...
    void throws_exception_on_invalid() {
        assertThrows(PatchException.class, () -> TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.invalid", "oldEntryRegex", "currentEntry", "newEntry", "newEntry")), config));
    }

    @Test
    void throws_exception_on_no_matching_regex() {
        assertThrows(PatchException.class, () -> TRANSFORMER.transform(new PatcherOptionsImpl(Map.of("key", "section.myList", "oldEntryRegex", "invalidRegex", "newEntry", "newEntry")), config));
    }
}
