...
/**
 * <p>
 * Superclass of all action schedulers.
 ...
 * the referenced actions shall be executed using {@link #executeActions(Schedule)}.
...
public abstract class Scheduler<S extends Schedule, T> {
     ...
     * It executes all actions that should be run by the schedule.
     *
     * @param schedule a schedule that reached execution time, providing a list of actions to run
     */
    protected void executeActions(final S schedule) {
        log.debug(schedule.getId().getPackage(), "Schedule '" + schedule.getId() + "' runs its actions...");
        questTypeApi.actions(null, schedule.getActions());
    }
