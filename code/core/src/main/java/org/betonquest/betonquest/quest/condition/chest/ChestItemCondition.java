...
public class ChestItemCondition implements NullableCondition {
     ...
     */
    public ChestItemCondition(final Variable<Location> loc, final Variable<List<QuestItemWrapper>> items) {
        this.items = items;
        this.loc = loc;
    }
    ...
    public boolean check(@Nullable final Profile profile) throws QuestException {
        ...
        int counter = 0;
        final List<QuestItemWrapper> resolvedItems = items.getValue(profile);
        for (final QuestItemWrapper item : resolvedItems) {
