...
public class CompassCommand implements CommandExecutor {
     ...
     */
    public CompassCommand(final ConfigAccessor config, final Variables variables, final PluginMessage pluginMessage, final ProfileProvider profileProvider) {
        this.config = config;
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        if ("compass".equalsIgnoreCase(cmd.getName())) {
            if (sender instanceof Player) {
                new Backpack(config, variables, pluginMessage, profileProvider.getProfile((Player) sender), DisplayType.COMPASS);
            }
            return true;
        }
        return false;
