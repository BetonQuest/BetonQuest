...
public class RandomNumberVariableFactory implements PlayerVariableFactory, PlayerlessVariableFactory {
    ...
    private Variable<Number> parseFirst(final Instruction instruction) throws QuestException {
        ...
        if (start.startsWith("{")) {
            ...
            if (start.contains("~")) {
                return instruction.get(start.substring(0, start.indexOf('~')), DefaultArgumentParsers.NUMBER);
            } else {
                final String next = instruction.next();
                return instruction.get(start + '.' + next.substring(0, next.indexOf('~')), DefaultArgumentParsers.NUMBER);
            }
        }
    }
    ...
    private Variable<Number> parseSecond(final Instruction instruction) throws QuestException {
        ...
        if (start.startsWith("{")) {
            ...
            return instruction.get(instruction.hasNext() ? start + '.' + instruction.next() : start, DefaultArgumentParsers.NUMBER);
        }
    }
    ...
    private Variable<Number> parseToVariable(final String start, final Instruction instruction) throws QuestException {
        ...
        while (true) {
            ...
            if (current.contains("}")) {
                ...
                return instruction.get(builder.toString(), DefaultArgumentParsers.NUMBER);
            } else {
