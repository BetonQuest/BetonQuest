 ...
 */
public abstract class BaseCommandActionFactory implements PlayerActionFactory {
     ...
     */
    public List<Argument<String>> parseCommands(final Instruction instruction) throws QuestException {
        final List<Argument<String>> commands = new ArrayList<>();
        ...
        for (final String rawCommand : rawCommands) {
            commands.add(instruction.chainForArgument(rawCommand).string().get());
        }
        return commands;
    }
}
