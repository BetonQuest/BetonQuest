...
- Changed quest package structure
    ...
    - Quest packages can now contain quest packages in a sub folder
    - `main.yml` was renamed to `package.yml`
    - File can now be named as wanted except `package.yml`, this indicates a quest package
    - `events`, `objectives`, `conditions`, `journal` and `items` are now defined in a config section matching there
      names
    - `conversations` and `menus` are now defined in a config section matching there names and a unique identifier like
