...
class JournalEventFactoryIntegrationTest {
    ...
    private QuestEventFactoryAdapter createJournalEventFactory(final BetonQuestLogger logger) {
        ...
        final JournalEventFactory journalEventFactory = new JournalEventFactory(new SingletonLoggerFactory(logger), dataStorage, InstantSource.fixed(now), saver, profileProvider);
