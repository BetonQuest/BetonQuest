...
public class CheckCondition extends Condition {
	...
	private Condition createCondition(final String instruction) throws InstructionParseException {
		...
		if(conditionClass == null) {
			...
			throw new InstructionParseException("Condition type " + parts[0] + " is not registered, check if it's" + " spelled correctly in internal condition");
		}
		try {
			return conditionClass.getConstructor(Instruction.class).newInstance(new Instruction(this.instruction.getPackage(), null, instruction));
