...
public class MenuConvIO extends ChatConvIO {
	...
	public MenuConvIO(final Conversation conv, final String playerID) {
		...
		// Load Configuration from custom.yml with some sane defaults, loading
		// our current package last
		for(final ConfigPackage pack : Stream.concat(Config.getPackages().values().stream().filter(p -> p != conv.getPackage()), Stream.of(conv.getPackage())).collect(
				Collectors.toList())) {
		...
		// Sort out Controls
		try {
			for(final CONTROL control : Arrays.stream(configControlCancel.split(",")).map(String::toUpperCase).map(CONTROL::valueOf).collect(Collectors.toList())) {
		...
		try {
			for(final CONTROL control : Arrays.stream(configControlSelect.split(",")).map(String::toUpperCase).map(CONTROL::valueOf).collect(Collectors.toList())) {
		...
		try {
			for(final CONTROL control : Arrays.stream(configControlMove.split(",")).map(String::toUpperCase).map(CONTROL::valueOf).collect(Collectors.toList())) {
				if(!controls.containsKey(control)) {
	...
	private void start() {
		// Create something painful looking for the player to sit on and make it
		// invisible.
		...
		// Intercept Packets
		packetAdapter = new PacketAdapter(BetonQuest.getInstance(), ListenerPriority.HIGHEST, PacketType.Play.Client.STEER_VEHICLE, PacketType.Play.Server.ANIMATION) {
	...
	public void setNpcResponse(final String npcName, final String response) {
		...
		formattedNpcName = configNpcNameFormat.replace("{npc_name}", npcName);
	}
	...
	protected void updateDisplay() {
		...
		// NPC Text
		final String msgNpcText = configNpcText.replace("{npc_text}", npcText).replace("{npc_name}", npcName);

		final List<String> npcLines = Arrays.stream(LocalChatPaginator.wordWrap(Utils.replaceReset(StringUtils.stripEnd(msgNpcText, "\n"), configNpcTextReset),
				configLineLength, configNpcWrap)).collect(Collectors.toList());

		// Provide for as many options as we can fit but if there is lots of
		// npcLines we will reduce this as necessary down to a minimum of 1.
		...
		// Displaying options is tricky. We need to deal with if the selection
		// has moved, multi-line options and less space for all options due to
		// npc text
		...
		for(int i = 0; i < options.size() && linesAvailable > (i < 2 ? 0 : 1); i++) {
			...
			if(i == 0) {
				final String optionText = configOptionSelected.replace("{option_text}", options.get(optionIndex + 1)).replace("{npc_name}", npcName);

				optionLines = Arrays.stream(LocalChatPaginator.wordWrap(Utils.replaceReset(StringUtils.stripEnd(optionText, "\n"), i == 0 ? configOptionSelectedReset
						: configOptionTextReset), configLineLength, configOptionSelectedWrap)).collect(Collectors.toList());

			} else {
				final String optionText = configOptionText.replace("{option_text}", options.get(optionIndex + 1)).replace("{npc_name}", npcName);

				optionLines = Arrays.stream(LocalChatPaginator.wordWrap(Utils.replaceReset(StringUtils.stripEnd(optionText, "\n"), i == 0 ? configOptionSelectedReset
						: configOptionTextReset), configLineLength, configOptionWrap)).collect(Collectors.toList());
	...
	/**
	 * @return if this conversationIO should send messages to the player when
	...
	public void playerItemHeldEvent(final PlayerItemHeldEvent event) {
		...
		// Cheat and assume the closest distance between previous and new slots
		// is the direction scrolled
