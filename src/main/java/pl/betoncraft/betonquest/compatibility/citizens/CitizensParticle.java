...
public class CitizensParticle extends BukkitRunnable {
	...
	public CitizensParticle() {
		...
		for(final ConfigPackage pack : Config.getPackages().values()) {
			...
			if(interval <= 0) {
				LogUtils.getLogger().log(Level.WARNING, "Could not load npc effects of package " + pack.getName() + ": " + "Check interval must be bigger than 0.");
				return;
			}
			...
			for(final String key : section.getKeys(false)) {
				...
				// if the key is not a configuration section then it's not an
				// effect
				if(settings == null) {
					continue;
				}
	...
	private void checkConditions() {
		...
		for(final Player player : Bukkit.getOnlinePlayers()) {
			...
			// handle all effects
			effects : for(final Effect effect : effects) {
				...
				// assign this effect to all NPCs which don't have already
				// assigned effects
	...
	private void activateEffects() {
		...
		for(final Player player : Bukkit.getOnlinePlayers()) {
			...
			for(final Entry<Integer, Effect> entry : assignments.entrySet()) {
				...
				// skip if there are no such NPC or it's not spawned or not
				// visible
				if(npc == null || !npc.isSpawned() || npc.getEntity().getWorld() != player.getWorld() || (NPCHider.getInstance() != null && NPCHider.getInstance()
						.isInvisible(player, npc))) {
					continue;
				}
				...
				// fire the effect
				EffectLibIntegrator.getEffectManager().start(effect.name, effect.settings, new DynamicLocation(loc, null), new DynamicLocation(null, null),
						(ConfigurationSection) null, player);
			}
		}
	}
