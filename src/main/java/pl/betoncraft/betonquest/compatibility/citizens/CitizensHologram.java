 ...
 * <p>
 * Some care is taken to optimize how holograms are displayed. They are
 * destroyed when not needed, shared between players and
 * we only have a fast update when needed to ensure they are relative to the NPC
 * position
 */
...
public class CitizensHologram extends BukkitRunnable implements Listener {
	...
	public CitizensHologram() {
		...
		Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(BetonQuest.getInstance(), () -> {
			...
			for(final ConfigPackage pack : Config.getPackages().values()) {
				...
				if(interval <= 0) {
					LogUtils.getLogger().log(Level.WARNING, "Could not load npc holograms of package " + pack.getName() + ": " + "Check interval must be bigger than 0.");
					return;
				}
				...
				for(final String key : section.getKeys(false)) {
					...
					// if the key is not a configuration section then it's not a
					// hologram
					if(settings == null) {
						continue;
					}
					...
					try {
						...
						hologramConfig.vector = new Vector(Double.parseDouble(vectorParts[0]), Double.parseDouble(vectorParts[1]), Double.parseDouble(vectorParts[2]));
					} catch(final NumberFormatException e) {
					...
					if(rawConditions != null) {
						for(final String part : rawConditions.split(",")) {
							try {
								...
								LogUtils.getLogger().log(Level.WARNING, "Error while loading " + part + " condition for hologram " + pack.getName() + "." + key + ": " + e
										.getMessage());
								LogUtils.logThrowable(e);
							}
						}
	...
	private void updateHolograms() {
			...
			for(final NPCHologram npcHologram : npcs.get(npcID)) {
				...
				for(final Player player : Bukkit.getOnlinePlayers()) {
					...
					if(visible) {
						...
						if(npcHologram.hologram == null) {
							final Hologram hologram = HologramsAPI.createHologram(BetonQuest.getInstance(), npc.getStoredLocation().clone().add(
									npcHologram.config.vector));
							...
							for(final String line : npcHologram.config.settings.getStringList("lines")) {
								if(line.startsWith("item:")) {
									try {
										...
										LogUtils.getLogger().log(Level.WARNING, "Could not parse item " + line.substring(5) + " hologram: " + e.getMessage());
										LogUtils.logThrowable(e);
									} catch(final ObjectNotFoundException e) {
										LogUtils.getLogger().log(Level.WARNING, "Could not find item in " + line.substring(5).split(":")[0] + " hologram: " + e
												.getMessage());
										LogUtils.logThrowable(e);

										// TODO Remove this code in the version
										// 1.13 or later
										// This support the old implementation
										// of Items
										final Material material = Material.matchMaterial(line.substring(5));
										if(material != null) {
											LogUtils.getLogger().log(Level.WARNING,
													"You use the Old method to define a hover item, this still work, but use the new method,"
						...
						// We do this a tick later to work around a bug where
						// holograms simply don't appear
		...
		if(npcUpdater) {
			if(updater == null) {
				// The updater only runs when at least one hologram is visible
				// to ensure it stays relative to npc location
