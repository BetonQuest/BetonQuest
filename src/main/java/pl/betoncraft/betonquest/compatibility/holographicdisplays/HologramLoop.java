...
public class HologramLoop {
	...
	/**
	 * Starts a loop, which checks hologram conditions and shows them to
	 * players.
	 */
	public HologramLoop() {
				...
				if(rawConditions != null) {
					...
					for(int i = 0; i < conditions.length; i++) {
						try {
							...
							LogUtils.getLogger().log(Level.WARNING, "Error while loading " + parts[i] + " condition for hologram " + packName + "." + key + ": " + e
									.getMessage());
				...
				for(final String line : lines) {
					...
					if(line.startsWith("item:")) {
						try {
							...
							// TODO Remove this code in the version 1.13 or
							// later
		...
		runnable = new BukkitRunnable() {
			...
			public void run() {
				for(final Player player : Bukkit.getOnlinePlayers()) {
					...
					holograms : for(final Entry<Hologram, ConditionID[]> entry : holograms.entrySet()) {
		...
		runnable.runTaskTimer(BetonQuest.getInstance(), 20, BetonQuest.getInstance().getConfig().getInt("hologram_update_interval", 20 * 10));
	}

	/**
	 * Cancels hologram updating loop and removes all BetonQuest-registered
	 * holograms.
	 */
	public void cancel() {
		if(runnable == null)
