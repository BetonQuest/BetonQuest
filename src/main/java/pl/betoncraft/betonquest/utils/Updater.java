...
public class Updater {
            ...
            public void run() {
                ...
                try {
                    ...
                    for (int i = 0; i < json.length(); i++) {
                        // read all info from each release and put it into the hashmap
                        ...
                        if (version.coreVersion == locVer.coreVersion) {
                            if (version.majorVersion >= highestRelease.majorVersion) {
                                // if the version is higher than what we found already, make sure bugfix version is also updated
                                if (version.majorVersion > highestRelease.majorVersion) {
                    ...
                    // if the update targets are different that current version, get their urls
                    ...
                    if (!highestRelease.equals(locVer) || isDevBuild) {
                        // if it's dev build, it will try to get the address of the update. if there is no such
                        // address in the map it means that there is no update yet
                        for (Version v : remoteVersions.keySet()) {
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        if (updateBugFixes && bugfixAddress != null) {
                            Debug.broadcast("Found bugfix version: " + remoteBugfix + ", it will be downloaded on next restart/reload.");
                        }
                        if (notifyNewRelease && releaseAddress != null && !remoteRelease.equals(remoteBugfix)) {
                            Debug.broadcast("Found new release: " + remoteRelease + ", use '/q update' to download it.");
                        }
                        if (notifyDevBuild && devBuildAddress != null) {
                            Debug.broadcast("Found new development build: " + remoteDevBuild + ", use '/q update --dev' to download it.");
                        }
                    }
                }.runTask(plugin);
            }
    ...
    private void load() {
        ...
        // if it's an official dev build and there is no dev option, try to add it
        if (isDevBuild && isOfficial) {
    ...
    private Version parseTagVersion(String tag) throws Exception {
        ...
        if (numbers.length > 2) version.bugfixVersion = Integer.parseInt(numbers[2]);
        return version;
        ...
        if (enabled && releaseAddress != null) {
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    bugfixAddress = null; // this will prevent bugfixes from overwriting the release
                }
            }.runTaskAsynchronously(plugin);
            return true;
        }
    ...
    private class Version {
        ...
        public boolean equals(Object o) {
            if (o instanceof Version) {
                Version v = (Version) o;
                return coreVersion == v.coreVersion && majorVersion == v.majorVersion && bugfixVersion == v.bugfixVersion;
            }
            return false;
        }
    }
