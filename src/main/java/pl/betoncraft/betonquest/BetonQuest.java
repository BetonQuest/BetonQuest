...
public class BetonQuest extends JavaPlugin {
	...
	public static boolean condition(final String playerID, final ConditionID conditionID) {
		...
		LogUtils.getLogger().log(Level.FINE, (isMet ? "TRUE" : "FALSE") + ": " + (conditionID.inverted() ? "inverted" : "") + " condition " + conditionID + " for player "
	...
	public static void event(final String playerID, final EventID eventID) {
		...
		if(playerID == null) {
			...
			LogUtils.getLogger().log(Level.FINE, "Firing event " + eventID + " for " + PlayerConverter.getName(playerID));
		}
		try {
			event.fire(playerID);
		} catch(final QuestRuntimeException e) {
	...
	public static void newObjective(final String playerID, final ObjectiveID objectiveID) {
		...
		if(objective.containsPlayer(playerID)) {
			LogUtils.getLogger().log(Level.FINE, "Player " + PlayerConverter.getName(playerID) + " already has the " + objectiveID + " objective");
			return;
		}
	 ...
	 */
	public static Variable createVariable(final ConfigPackage pack, final String instruction) throws InstructionParseException {
		VariableID ID;
		try {
		...
		try {
			final Variable variable = variableClass.getConstructor(Instruction.class).newInstance(new VariableInstruction(pack, null, instruction));
	...
	public void onEnable() {
		...
		this.database = new MySQL(this, getConfig().getString("mysql.host"), getConfig().getString("mysql.port"), getConfig().getString("mysql.base"), getConfig()
				.getString("mysql.user"), getConfig().getString("mysql.pass"));
	...
	public void loadData() {
		...
		for(final ConfigPackage pack : Config.getPackages().values()) {
			...
			for(final String key : eConfig.getKeys(false)) {
				...
				if(eventClass == null) {
					...
					// an error
					LogUtils.getLogger().log(Level.WARNING, "Event type " + type + " is not registered, check if it's" + " spelled correctly in '" + ID + "' event.");
					continue;
				}
				try {
					final QuestEvent event = eventClass.getConstructor(Instruction.class).newInstance(ID.generateInstruction());
					events.put(ID, event);
			...
			for(final String key : cConfig.getKeys(false)) {
				...
				if(conditionClass == null) {
					LogUtils.getLogger().log(Level.WARNING, "Condition type " + type + " is not registered," + " check if it's spelled correctly in '" + ID
							+ "' condition.");
					continue;
				}
				try {
					final Condition condition = conditionClass.getConstructor(Instruction.class).newInstance(ID.generateInstruction());
			...
			for(final String key : oConfig.getKeys(false)) {
				...
				if(objectiveClass == null) {
					LogUtils.getLogger().log(Level.WARNING, "Objective type " + type + " is not registered, check if it's" + " spelled correctly in '" + ID
							+ "' objective.");
					continue;
				}
				try {
					final Objective objective = objectiveClass.getConstructor(Instruction.class).newInstance(ID.generateInstruction());
			...
			for(final String convName : pack.getConversationNames()) {
				if(convName.contains(" ")) {
					LogUtils.getLogger().log(Level.WARNING, "Conversation name cannot contain spaces: '" + convName + "' (in " + packName + " package)");
					continue;
				}
				try {
					conversations.put(pack.getName() + "." + convName, new ConversationData(pack, convName));
				} catch(final InstructionParseException e) {
					LogUtils.getLogger().log(Level.WARNING, "Error in '" + packName + "." + convName + "' conversation: " + e.getMessage());
					LogUtils.logThrowable(e);
				} catch(final Exception e) {
					LogUtils.logThrowableReport(e);
				}
		...
		// done
		LogUtils.getLogger().log(Level.INFO, "There are " + conditions.size() + " conditions, " + events.size() + " events, " + objectives.size() + " objectives and "
