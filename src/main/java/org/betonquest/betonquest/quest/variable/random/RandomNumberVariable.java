...
public class RandomNumberVariable implements NullableVariable {
     ...
     */
    public RandomNumberVariable(final VariableNumber low, final VariableNumber high, final boolean fractional, @Nullable final DecimalFormat format) {
        this.low = low;
        this.high = high;
    ...
    public String getValue(@Nullable final Profile profile) throws QuestException {
        try {
            if (fractional) {
                final double value = ThreadLocalRandom.current().nextDouble(low.getValue(profile).doubleValue(), high.getValue(profile).doubleValue());
                if (format != null) {
