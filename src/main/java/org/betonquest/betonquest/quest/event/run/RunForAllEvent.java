...
public class RunForAllEvent implements StaticEvent {
     ...
     */
    public RunForAllEvent(final Supplier<? extends Iterable<? extends Profile>> profileCollectionSupplier, final BetonQuestAPI questAPI, final List<EventID> events, final List<ConditionID> conditions) {
    ...
    public void execute() throws QuestException {
        for (final Profile profile : profileCollectionSupplier.get()) {
            if (conditions.isEmpty() || questAPI.conditions(profile, conditions.toArray(new ConditionID[0]))) {
