...
public class PickRandomEvent implements NullableEvent {
     ...
     */
    public PickRandomEvent(final List<RandomEvent> events, @Nullable final VariableNumber amount, final BetonQuestAPI questAPI) {
        this.events = events;
        this.amount = amount;
    ...
    public void execute(@Nullable final Profile profile) throws QuestException {
        ...
        while (pick > 0 && !resolvedEvents.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                random -= event.chance();
                if (random < 0) {
                    questAPI.event(profile, event.eventID());
                    iterator.remove();
                    total -= event.chance();
                    break;
                }
