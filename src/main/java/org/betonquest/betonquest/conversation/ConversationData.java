            ...
            if (conv.contains("prefix")) {
                if (conv.isConfigurationSection("prefix")) {
                    for (final String lang : conv.getConfigurationSection("prefix").getKeys(false)) {
                        final String pref = conv.getConfigurationSection("prefix").getString(lang);
                        if (pref != null && !pref.isEmpty()) {
                            final String value = new VariableString(variableProcessor, pack, pref).getValue(null);
                            inlinePrefix.put(lang, value);
                        }
                    }
                    ...
                    final String pref = conv.getString("prefix");
                    if (pref != null && !pref.isEmpty()) {
                        final String value = new VariableString(variableProcessor, pack, pref).getValue(null);
                        inlinePrefix.put(defaultLang, value);
                    }
                }
            }
        }
