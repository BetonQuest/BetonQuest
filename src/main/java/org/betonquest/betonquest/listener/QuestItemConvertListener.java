...
import static org.betonquest.betonquest.util.Utils.QUEST_ITEM_KEY;

/**
 * Adds the {@link Utils#QUEST_ITEM_KEY} to all "legacy" Quest Items
...
public class QuestItemConvertListener implements Listener {
    ...
    /**
     * If the ItemStacks in inventories should be updated.
     */
     ...
     * @param config          the config for the listener
     ...
     */
    public QuestItemConvertListener(final BetonQuestLogger log, final ConfigAccessor config,
    ...
    /**
     * Reloads the enabled status when BQ reloads.
     *
     * @param event the load event
     */
    @EventHandler
    public void onReload(final LoadDataEvent event) {
        this.enabled = config.getBoolean("join_item_updater");
    }
}
