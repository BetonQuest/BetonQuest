...
public class QuestsIntegrator implements Integrator {
    ...
    public void hook() {
        ...
        questsInstance.getCustomRewards().add(new EventReward(loggerFactory.create(EventReward.class), plugin.getQuestAPI()));
        questsInstance.getCustomRequirements().add(new ConditionRequirement(
                loggerFactory.create(ConditionRequirement.class), plugin.getQuestAPI()));
    }
