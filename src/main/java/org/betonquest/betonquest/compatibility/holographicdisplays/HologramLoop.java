...
public class HologramLoop {
    ...
    public HologramLoop() {
        ...
        for (final QuestPackage pack : Config.getPackages().values()) {
            ...
            for (final String key : section.getKeys(false)) {
                ...
                final int checkInterval = section.getInt(key + ".check_interval",
                        BetonQuest.getInstance().getPluginConfig().getInt("hologram_update_interval", 10 * 20));
                if (rawLocation == null) {
                ...
                for (final String line : lines) {
                    ...
                    if (line.startsWith("item:")) {
                        ...
                        if (!pointName.contains(".")) {
                            pointName = new StringBuilder(packName).append('.').append(pointName).toString();
                        }
    ...
    public void cancel() {
        for (final HologramRunner runner : HologramRunner.getRunners()) {
            ...
            for (final HologramWrapper hologramWrapper : runner.getHolograms()) {
                hologramWrapper.getHologram().getVisibilitySettings().clearIndividualVisibilities();
                hologramWrapper.getHologram().delete();
            }
        }
    ...
    private static final class HologramRunner {
        ...
        public static void addHologram(final HologramWrapper hologram) {
            if (!RUNNERS.containsKey(hologram.getInterval())) {
                RUNNERS.put(hologram.getInterval(), new HologramRunner(hologram.getInterval()));
            }
            RUNNERS.get(hologram.getInterval()).holograms.add(hologram);
