...
Each BetonQuest variable can be displayed on a hologram in a text line. These variables use the same definition syntax as in conversations such that; '`%package:variable%`'. Where the `package` part is optional if the hologram is defined in the same package as the variable. If you wish to refer to a variable that is *not* in the same package as the hologram, then you must use a `package` prefix. See [References](Reference.md#top-level-packages) for how to reference across packages. Make sure to use a *top-level downwards* approach when referring to packages. 
For example '`%topPackage-subPackage:point.example_point.amount%`'

!!! warning "Potential lags"
    The HolographicDisplays documentations warns against using too many individual hologram variables since they are rendered for each player individually. If you are using HolographicDisplays, to save resources, it is recommended to use global variables with the **'$'** symbol instead of the **'%'**. This does mean, however that only player-unrelated variables such as `globalpoint` and `globaltag` can be used. Using them with player specific variables will not necessarily throw errors but can produce weird results.
