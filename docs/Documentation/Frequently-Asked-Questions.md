...
Either use a folder action that holds all tag-deleting actions.
Or use a `run` action in which you specify all tag-deleting actions at once.
...
You will be able to see which actions etc. loaded and which didn't with the reasons why.
...
YAML Syntax Errors are the HUGE red lines that you see when you do `/bq reload`.
...
You can do that with `die` objective - simply start it for the player at the beginning of the quest and make it fire actions that
...
## Racing with folder action

Since `folder` action can run `tag` actions even for offline players you can create races. Create `location` objective
...
Now when the race starts fire `folder` action with the amount of time you want to give your players to complete the race.
This action should set "race_failed" tag. If the player reaches the location before this tag is set,
he will fire all actions in that `location` objective, but if the time has passed, the objective will not be completed.
...
Imagine you want to lie to the NPC and he has 15% chance of believing you completely, 35% of being suspicious and 50% of not believing at all.
...
Name the NPC "Quester", add one option for each quest etc. Now you need an objective which will start this conversation using `conversation` action.
