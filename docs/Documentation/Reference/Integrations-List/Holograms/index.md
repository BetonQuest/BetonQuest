...
[`hologram.default` option in the "_config.yml_"](../../Plugin-Config.md#hologram-hologram-settings)
...
must specify a [package](../../../Basics/Packages-&-Templates.md) before the `placeholder`.
...
??? warning "HolographicDisplays: Potential lags"
    ...
    for each player individually. If you are using HolographicDisplays to save resources, it is recommended to 
    minimize the use of player-dependent placeholders.
...
number of ticks you want to pass between updates (one second is 20 ticks). Setting it to 0 or negative numbers will
deactivate automatic updates at all, use the [`hologramUpdate` action](../../Actions-List.md#updatehologram) then.
