...
??? info "Automated Migration"
    *The migration is automated. You shouldn't have to do anything.*
    ...
    - Quest packages can now contain nested quest packages in sub folders. You can also have any file and folder
      ...
      [quest package](../../Basics/Packages-&-Templates.md).
...
??? info "Automated Migration"
    *The migration is automated. You shouldn't have to do anything.*
    ...
    - All your static events (now called actions) need to be converted to the new scheduling system.
      The [`realtime-daily`](../../Advanced/Schedules.md#daily-realtime-schedule-realtime-daily) schedule makes this easy:
...
The `smelt` objective now requires a [quest item](../../Advanced/Items.md) instead of a BlockSelector.
...
??? info "Automated Migration"
    *The migration is automated. You shouldn't have to do anything.*
    ...
    See the [objective's documentation](../../Reference/Integrations-List/MMO/MMOLib.md#mmoskill) for more information.
...
??? info "Automated Migration"
    *The migration is automated. You shouldn't have to do anything.*
    ...
    Until now if you used non-static events (now called actions) in a [schedule](../../Advanced/Schedules.md), they were executed for every player that was online at the time.  
    ...
    To fix this wrap the event (now called action) in a [`runForAll` event](../../Reference/Actions-List.md#runforall):
    ...
    2. `notify_goodNight` is a [static event](../../Advanced/Schedules.md#player-independent-actions), so no need to wrap it in `runForAll`.
...
??? info "Automated Migration"
    *The migration is automated. You shouldn't have to do anything.*
    ...
    Both got merged into the [RemoveEntity event](../../Reference/Actions-List.md#removeentity),
...
[quoting](../../Basics/Quoting-&-YAML.md#quoting) works.
