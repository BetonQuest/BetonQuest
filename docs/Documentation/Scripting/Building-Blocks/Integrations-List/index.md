---
icon: material/handshake
tags:
  - Integration
---
# Integrations List

In total **@snippet:constants:totalIntegratedPluginsNumber@** plugins are integrated with BetonQuest.
Mostly provided by BetonQuest itself, some are provided by other plugins.
The categories are just to make it easier for you to find the plugins you're looking for and do not represent a strict order.

## Provided by BetonQuest

<div class="grid cards" markdown>
 -   [NPCs](./NPCs/index.md)
 
     ---
    - [Citizens](./NPCs/Citizens.md)
    - [FancyNpcs](./NPCs/FancyNpcs.md)
    - [MythicMobs](./NPCs/MythicMobs.md)
    - [ZNPCsPlus](./NPCs/ZNPCsPlus.md)
    
 -   [Items](./Items/index.md)

     ---
    - [Brewery & BreweryX](./Items/Brewery.md)
    - [CraftEngine](./Items/CraftEngine.md)
    - [ItemsAdder](./Items/ItemsAdder.md)
    - [MMOItems](./Items/MMOItems.md)
    - [Nexo](./Items/Nexo.md)

-    [Holograms](./Holograms/index.md)

     ---
    - [DecentHolograms](./Holograms/DecentHolograms.md)
    - [HolographicDisplays](./Holograms/HolographicDisplays.md)

-    [Visuals](./Visuals/index.md)

     ---
    - [EffectLib](./Visuals/EffectLib.md)
    - [FakeBlock](./Visuals/FakeBlock.md)
    
-    [Scripting & Questing](./Scripting/index.md)

     ---
    - [Denizen](./Scripting/Denizen.md)
    - [Quests](./Scripting/Quests.md)
    - [Skript](./Scripting/Skript.md)

-    [Economy](./Economy/index.md)

     ---
    - [JobsReborn](./Economy/JobsReborn.md)
    - [Shopkeepers](./Economy/Shopkeepers.md)
    - [Vault](./Economy/Vault.md)
    
-    [MMO](./MMO/index.md)

     ---
    - [AuraSkills](./MMO/AuraSkills.md)
    - [Heroes](./MMO/Heroes.md)
    - [Magic](./MMO/Magic.md)
    - [mcMMO](./MMO/McMMO.md)
    - [MMOCore](./MMO/MMOCore.md)
    - [MMOLib](./MMO/MMOLib.md)
    - [Fabled](./MMO/Fabled.md)

-    [Administration](./Administration/index.md)

     ---
    - [LuckPerms](./Administration/LuckPerms.md)
    - [PacketEvents](./Administration/PacketEvents.md)
    - [PlaceholderAPI](./Administration/PlaceholderAPI.md)
    - [RedisChat](./Administration/RedisChat.md)
    - [WorldEdit & FastAsyncWorldEdit](./Administration/WorldEdit.md)
    - [WorldGuard](./Administration/WorldGuard.md)

-    [Miscellaneous](./Miscellaneous/index.md)

     ---
    - [TrainCarts](./Miscellaneous/TrainCarts.md)

</div>

## Provided by other plugins

- [BetonQuestAddon](https://modrinth.com/plugin/XvDcVrRl) – Adds additional integrations to BetonQuest.
- [CalebCompass](https://www.spigotmc.org/resources/82674/) – Compass HUD like skyrim.
- [Depenizen](https://github.com/DenizenScript/Depenizen) – Compatibility with Denizen.
- [HonnyCompass](https://github.com/honnisha/HonnyCompass) – Compass shown in the bossbar.
- [JourneyBetonQuest](https://modrinth.com/plugin/journeybetonquest) – Compatibility with Journey.
- [MythicDungeons](https://www.spigotmc.org/resources/102699/) – Dungeon generator.
- [NotQuests](https://www.spigotmc.org/resources/95872/) – Quest writing.
- [nuNPCDestinations](https://www.spigotmc.org/resources/13863/) – Gives more control to npc movement and destination.

These plugins require a clientside mod:

- [BetonQuestGUI](https://github.com/giovanni-bozzano/betonquest-gui-plugin) – Mod-based conversation IO.
- [NGVexJournal](https://www.spigotmc.org/resources/76938/) – Mod-based journal.
