...
public class Utils
{
	...
	public static void backup()
	{
		...
		if(!backupDatabase(new File(instance.getDataFolder(), "database-backup.yml")))
		{
			LogUtils.getLogger().log(Level.WARNING, "There was an error during backing up the database! This does not affect"
					...
					+ " the database maually if you want to be extra safe, but it's not necessary if" + " you don't want to downgrade later.");
		}
		...
		// zip all the files
		String outputPath = backupFolder.getAbsolutePath() + File.separator + "backup-" + instance.getConfig().getString("version", null);
	...
	public static boolean backupDatabase(File databaseBackupFile)
	{
		...
		try
		{
			...
			{
					"objectives", "tags", "points", "journals", "player"
			};
		...
		try
		{
			...
			if(!(isOldDatabaseBackedUP = backupDatabase(new File(backupFolder, filename))))
			{
				LogUtils.getLogger().log(Level.WARNING, "There was an error during old database backup process. This means that"
						+ " if the plugin loaded new database (from backup), the old one would be lost " + "forever. Because of that the loading of backup was aborted!");
				return;
			}
			...
			if(objectives != null)
				for(String key : objectives.getKeys(false))
				{
					...
					{
							objectives.getString(key + ".id"), objectives.getString(key + ".playerID"), objectives.getString(key + ".objective"), objectives.getString(key
									+ ".instructions"),
					});
				}
			// load tags
			...
			if(tags != null)
				for(String key : tags.getKeys(false))
				{
					...
					{
							tags.getString(key + ".id"), tags.getString(key + ".playerID"), tags.getString(key + ".tag"),
					});
				}
			// load points
			...
			if(points != null)
				for(String key : points.getKeys(false))
				{
					...
					{
							points.getString(key + ".id"), points.getString(key + ".playerID"), points.getString(key + ".category"), points.getString(key + ".count"),
					});
				}
			// load journals
			...
			if(journals != null)
				for(String key : journals.getKeys(false))
				{
					...
					{
							journals.getString(key + ".id"), journals.getString(key + ".playerID"), journals.getString(key + ".pointer"), journals.getString(key
									+ ".date"),
					});
				}
			// load backpack
			...
			if(backpack != null)
				for(String key : backpack.getKeys(false))
				{
					...
					{
							backpack.getString(key + ".id"), backpack.getString(key + ".playerID"), backpack.getString(key + ".instruction"), backpack.getString(key
									+ ".amount"),
					});
				}
			// load player
			...
			if(player != null)
				for(String key : player.getKeys(false))
				{
					...
					{
							player.getString(key + ".id"), player.getString(key + ".playerID"), player.getString(key + ".language"), player.getString(key
									+ ".conversation")
					});
				}
		...
		catch(Exception e)
		{
			if(isOldDatabaseBackedUP)
			{
				LogUtils.getLogger().log(Level.WARNING, "Your database probably got corrupted, sorry for that :( The good news"
						+ " is that you have a backup of your old database, you can find it in backups" + " folder, named as " + filename
						+ ". You can try to use it to load the " + "backup, but it will probably have the same effect.");
			}
			else
			{
				LogUtils.getLogger().log(Level.WARNING, "There was an error during database loading, but fortunatelly the "
						...
						+ "again, and if the problem persists you should contact the developer to find" + " a solution.");
			}
	...
	public static boolean isQuestItem(ItemStack item)
	{
		...
		return item.hasItemMeta() && item.getItemMeta().hasLore() && item.getItemMeta().getLore().contains(Config.getMessage(Config.getLanguage(), "quest_item"));
	}
	...
	/**
	 * Resets any color resets to def. Also ensures any new lines copy the
	 * colours and format from the previous line
	 *
	 * @param pages
	 *            multiple pages to process
	 * @param def
	 *            default color code to use instead of resetting; use null for
	 *            regular reset code
	 ...
	 * alternate color codes with {@code &}
	 * <p>
	 * {@code format(string, false, false)} will return the string with no
	 * formatting done
	 *
	 * @param string
	 *            the input string
	 * @param colorCodes
	 *            if alternate color codes should be resolved
	 * @param lineBreaks
