...
releasePublish() {
  ...
  git tag v"${CURRENT_VERSION}" HEAD > /dev/null

  echo "    Push version tag..."
  git push "${RELEASE_REMOTE_REPOSITORY}" v"${CURRENT_VERSION}" > /dev/null
...
setupPublish() {
  ...
  mvn versions:set-property -DgenerateBackupPoms=false -Dproperty=version -DnewVersion="${NEW_VERSION}" > /dev/null

  echo "    Update CHANGELOG.md file..."
  NEW_CHANGELOG="## \[Unreleased\] - \${current-date}\n### Added\n### Changed\n### Deprecated\n### Removed\n### Fixed\n### Security\n"
  sed -i "s~## \[Unreleased\] - \${current-date}~${NEW_CHANGELOG}\n## \[${CURRENT_VERSION}\] - ${SETUP_TIME}~g" CHANGELOG.md > /dev/null

  echo "    Commit changed files..."
  git commit --all --message="Version bump to ${NEW_VERSION}" > /dev/null

  echo "    Push committed changes..."
  git push "${SETUP_REMOTE_REPOSITORY}" "HEAD:${SETUP_REMOTE_BRANCH}" > /dev/null
